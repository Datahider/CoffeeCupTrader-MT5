// CCT_ATRProvider.mqh
#ifndef CCT_ATRPROVIDER_MQH
#define CCT_ATRPROVIDER_MQH

#include "CCT_Config.mqh"
#include "CCT_ProvidersBase.mqh"

class CCctATRProvider : public CCctProvider
{
private:
  string m_symbol;
...
public:
  CCctATRProvider(const string symbol, const ENUM_TIMEFRAMES tf, const int period)
  {
    m_symbol=symbol; m_tf=tf; m_period=period;
    m_handle = iATR(m_symbol, m_tf, m_period);
    if(m_handle==INVALID_HANDLE)
      Print("ATRProvider: failed to create handle for ", m_symbol);
  }

  ~CCctATRProvider() override
  {
    if(m_handle!=INVALID_HANDLE) IndicatorRelease(m_handle);
  }

  string Name() const override { return "ATR"; }
